...
def save_to_github_gist(results):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            return True, f"Results saved to GitHub!"
        else:
            return False, f"Failed to save to GitHub. Status: {response.status_code}"
...
def main():
    ...
    st.markdown("""
    ### Task:
    Thank you for participating in this evaluation! This will take about **10** minutes. The task is to evaluate the quality of reverbs generated by a text-to-reverb model, which takes a text description of a space and generates a reverb impulse response.

    ### Instructions:
    1. **Listen carefully** to both the dry and wetaudio samples
